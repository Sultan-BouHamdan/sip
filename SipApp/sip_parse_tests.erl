-module(sip_parse_tests).
-include_lib("eunit/include/eunit.hrl").
-include("sip.hrl").

get_from_test() ->
    Text = "From: Adam",
    Res = sip_parse:get_from(Text),
...
get_to_test() ->
    Text = "To: Robert",
    Res = sip_parse:get_to(Text),
    ?assertEqual("Robert", Res).
    
split_on_newline_test() ->
    Text = "A\nB",
    Res = sip_parse:split_on_new_line(Text),
    ?assertEqual({"A","B"}, Res).
...
split_on_space_test() ->
    Text = "C D",
    Res = sip_parse:split_on_space(Text),
    ?assertEqual({"C","D"}, Res).
...
split_request_into_headers_and_data_test() ->    
    Text = "REQUEST\nFrom: Adam\nTo: Robert\nDATA",
    Res = sip_parse:split_into_headers_and_data(Text),
    ?assertEqual(["REQUEST", 
                  {from,"Adam"},
                  {to,"Robert"},
                  {data,"DATA"}],
...
parse_sip_message_test() ->
    ...
    Res = sip_parse:parse_sip_message(Text),
